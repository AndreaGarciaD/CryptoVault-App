...
import { postRegisterAdmin } from "../../services/AuthService";
...
const AdminFormPage = () => {
    ...
    const enviarDatos = (e) => {
        ...
        if (!telefono) {
            newErrors.telefono.invalidNumber = 'El teléfono es requerido';
        } else {

            if (isNaN(telefono)) {
                newErrors.telefono.invalidNumber = 'El teléfono debe ser un número';
            }
        }
            ...
            <Row className="mt-3">
                <Col>
                    <Card>
                        <Card.Body>
                            <Card.Title>
                                <h1 className="custom-title">Registrar nuevo Administrador</h1>
                            </Card.Title>
                            <Form noValidate validated={validated} onSubmit={enviarDatos}>
                                ...
                                <FormGroup className="mt-2">
                                    <LabelBS text="Correo" />
                                    <FormControl required type="email" value={correo} onChange={(e) => {
                                        setCorreo(e.target.value)
                                    }} />
                                </FormGroup>
                                <FormGroup className="mt-2">
                                    <LabelBS text="Teléfono" />
                                    <FormControl required isInvalid={errors.telefono} value={telefono} onChange={(e) => {
                                        setTelefono(e.target.value)
                                    }} />
                                    {errors.telefono?.invalidNumber && <Form.Control.Feedback type="invalid">{errors.telefono.invalidNumber}</Form.Control.Feedback>}
                                </FormGroup>
                                <FormGroup className="mt-2">
